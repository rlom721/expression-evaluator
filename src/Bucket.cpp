...
// default constructor
Bucket::Bucket() : OFindex(-1), count(0), nextOpenSlot(0) {
...
Bucket::Bucket(string key, string data) : OFindex(-1), count(0), nextOpenSlot(0) {
  slots[0].setKey(key);
...
void Bucket::incrCount() { count++; }

// set OFindex to argument
void Bucket::setOFindex(int index) { 
  OFindex = index; 
}
