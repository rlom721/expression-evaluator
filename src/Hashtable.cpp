...
// template <class Bucket>
void Hashtable::hashRecords(string dataFileName) {
  ifstream inFile;
    ...
    while (getline(getData, line)) {
      ...
      if (!line.empty()) {
        ...
        // set OF index of a bucket
          ...
          buckets[bNum - 1].setOFindex(overflowPtr - 1);
        }
      }
    }
  }
...
// // template <class Bucket>
// template <class Bucket>
void Hashtable::ReportHT(string reportFileName, string reportName){
  ofstream outFile;                // for storing file data
...
// template <class Bucket>
void Hashtable::SearchHT(string searchFileName) {
...
// template <class Bucket>
void Hashtable::collisionReport() {
  ofstream outFile;         // for storing file data
  ...
  // loop over ALL buckets in hash table and write their contents to file
  for (int i = 0; i < CAP; i++) {
    outFile << "\tBucket " << i + 1 << "\t    " << buckets[i].getCount() << '\n';

    // exclude empty buckets (empty first slot) from average
    if (buckets[i].getCount() != 0) noCols++;

    totCols += buckets[i].getCount();
  }
